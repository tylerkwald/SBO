...
The conditional core damage frequency is the probability of core damage given an intiaing event happens. This allows for the analysis of the consequence that an event can have even if the event is exceedingly rare. To begin the analyssi, a boolean representation of the event tree is made using the sum of the Minimal Cut Sets followed by the simplification of the equation. The minimum cut sets are the paths that lead to core damage and are taken from \figref{fig:event treeeee}. The equation begins as follows:
