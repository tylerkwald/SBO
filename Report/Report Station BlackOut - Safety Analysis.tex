...
\subsection{Deterministic Safety Analysis: Worst Case Without RCP Leak}
\label{sec:pshrs_dsa}
...
conditions, the same conclusion holds \emph{a fortiori} for all the less
...
\paragraph{Decay heat removal.}
The reactor power curve (Figure~\ref{fig:reactor_power}) follows the
expected decay heat profile: an immediate drop at SCRAM from
3250\,MW$_{\text{th}}$ to a few percent of nominal power, followed by
the slow asymptotic decrease characteristic of fission-product and
actinide decay. At $t = 72$\,h the residual thermal power is
$\sim 15$\,MW, which is more than a factor two below the peak passive
load of 31.8\,MW used for the PSHRS sizing
(Table~\ref{tab:pshrs_design}). The system is therefore sized on the
worst instant of the passive phase, and operates with increasing
margin throughout the rest of the transient.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{../OurSytem/graphs/reactor_power.png}
    \caption{\centering Core Power}
    \label{fig:reactor_power}
\end{figure}

\paragraph{Core coverage.}
...
\paragraph{Pool inventory and HX submergence.}
...
and volume 4 around $t \approx 67$\,h. \textbf{At $t = 72$\,h,
...
\paragraph{Absence of AFW contribution.}
Figure~\ref{fig:afw_flow} confirms that the AFW mass flow is
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{../OurSytem/graphs/AFW_flow.png}
    \caption{\centering AFW Flow}
    \label{fig:afw_flow}
\end{figure}
...
\paragraph{Conclusion of the DSA.}
...
\begin{equation}
\text{CCDP}_{\text{new}} \approx 9.08 \times10^{-3},
\qquad
F(CD)_{\text{SBO,new}} \approx 1.99\times 10^{-7}\ \text{pry},
\end{equation}
corresponding to a reduction of approximately two orders of
magnitude (i.e.\ $\sim$99\%) with respect to the current
configuration. A more refined estimate, based on a dedicated
fault-tree analysis of the PSHRS components and on a more accurate
quantification of common-cause failures between the active and
passive heat-removal paths, is beyond the scope of the present
preliminary design and is recommended as a follow-up activity.
...
The proposed PSHRS is a fully passive natural-circulation loop
connecting the secondary side of each Steam Generator to an
elevated evaporation pool through an external heat exchanger.
The system is automatically actuated by the loss of the DC signal
at battery depletion ($t \approx 5$\,h), without requiring AC
power, AAC sources or operator intervention. Three design
modifications have been introduced with respect to the
preliminary configuration: a T-shaped pool geometry that
guarantees HX submergence even after extensive boil-off, an
enlarged atmospheric vent that maintains the pool at atmospheric
pressure during the entire transient, and a filtered actuation
logic that engages the system only at the actual end of the
active heat removal phase. The preliminary hydraulic sizing
yielded a natural-circulation margin of 1.08 and a required
pool volume of $\sim$2000\,m$^3$ (with 20\% design margin),
demonstrating the feasibility of the passive heat removal path
under worst-case decay heat conditions.\\

The RELAP5 simulation of the worst case among the sequences
without RCP seal leakage, performed under the conservative
assumption of total AFW unavailability from $t = 0$, confirmed
that the PSHRS is able to maintain the plant in a safe condition
for the full 72-hour SBO coping period. The core level remains
constant at $\sim$4.08\,m throughout the transient -- the core
is never uncovered -- and the primary system establishes a stable
two-phase natural circulation supported by condensation in the
external heat exchanger. The pool inventory is sufficient to
sustain decay heat removal for the entire mission time, with the
T-shaped geometry ensuring that the HX bundle remains submerged
at $t = 72$\,h. The acceptance criteria associated with the
hydraulic design bases (DB-1, DB-3, DB-4, DB-5) are therefore
satisfied by direct deterministic verification.
\newpage
