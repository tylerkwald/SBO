...
This section presents the results we expect based on the expected physical behavior of the plant. Each sequence is derived taking into account the success criteria of each TE and the most likely outcome in case of failure or success. They can be seen in \tabref{tab:sequences}.


\begin{table}[H]
\centering
\caption{Accident sequences expected. Y/N indicate success/failure
of each top event; dashes indicate branches not developed.}
\label{tab:sequences}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}cccccccc@{}}
\toprule
\textbf{Seq.} & \textbf{TE1} & \textbf{TE2} & \textbf{TE3} & \textbf{TE4} & \textbf{TE5} & \textbf{Outcome}\\
              & SL          & AFW         & LL          & Batt        & AC          &                  &                              \\
\midrule
1  & Y & Y & Y & Y & Y & SAFE \\
2  & Y & Y & Y & Y & N & CD   \\ 
...
\begin{table}[H]
\centering
\caption{Y/N indicate success/failure
of each top event; dashes indicate branches not developed. The PCT column
reports the maximum cladding temperature reached in the RELAP5 simulation;
core damage is declared when PCT exceeds the safety limit of 1477 K.}
\label{tab:sequences}
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}ccccccccc@{}}
\toprule
\textbf{Seq.} & \textbf{TE1} & \textbf{TE2} & \textbf{TE3} & \textbf{TE4} & \textbf{TE5} & \textbf{Outcome} & \textbf{PCT [K]} & \textbf{$t_\mathrm{CD}$ [h]} \\
              & SL          & AFW         & LL          & Batt        & AC          &                  &                  &                              \\
\midrule
1  & Y & Y & Y & Y & Y & SAFE       & TBD & --- \\
2  & Y & Y & Y & Y & N & CD         & TBD & TBD \\
3  & Y & Y & Y & N & Y & SAFE       & TBD & --- \\
4  & Y & Y & Y & N & N & CD         & TBD & TBD \\
5  & Y & N & -- & -- & -- & CD      & TBD & TBD \\
6  & N & Y & Y & Y & Y & SAFE       & TBD & --- \\
7  & N & Y & Y & Y & N & CD         & TBD & TBD \\
8  & N & Y & Y & N & Y & SAFE       & TBD & --- \\
9  & N & Y & Y & N & N & CD         & TBD & TBD \\
10 & N & Y & N & Y & Y & SAFE       & TBD & --- \\
11 & N & Y & N & Y & N & CD         & TBD & TBD \\
12 & N & Y & N & N & Y & \textbf{?} & TBD & to be confirmed \\
13 & N & Y & N & N & N & CD         & TBD & TBD \\
14 & N & N & -- & -- & -- & CD      & TBD & TBD \\
\bottomrule
\end{tabular}
\end{table}

\section{Probabilstic Safety Analysis}
...
\subsection{Mathmatical Representation}
...
\begin{flalign}
    F(I) & = F(LT \cup ST) \\ \nonumber
         & = 2\text{x}10^{-5} + 2\text{x}10^{-6} \\ \nonumber
         & =  2.2 \text{x} 10 ^ {-5} \text{ pry}
\end{flalign}
...
\begin{equation}
    P(A_d) = 4.6\text{x}10{-3} \nonumber
\end{equation}

The failure on run probality is calculated as follows:

\begin{flalign}
    P(A_r) & \approx  \lambda_f * t_m \\ \nonumber
           &  \approx 8.45\text{x}10^{-5}  \text{hr}^{-1} * 5 \text{hr} \\ \nonumber
           & \approx 4.225\text{x}10^{-5} 
\end{flalign}
...
The small leak event is assumed to happen in the case that the Westinghouse Shut Down Seal. A primilinary safety analysis of this component from Westinghouse gives it a conservative failure frequency of 1 in 100 cases giving us the follow probability:
